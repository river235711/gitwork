...
The GDS list is remembered in the user's session
(~/.pdkgui/session/SKIPPER.json); on first use it falls back to
config.page_file("SKIPPER") (default data/skipper.txt). Up to 10 rows. Per row:
...
class SkipperPage(BasePage):
    ...
    # ------------------------------------------------------------------
    def _load_rows(self):
        """Restore the saved GDS list; fall back to data/skipper.txt."""
        saved = config.load_json(config.user_global_file("SKIPPER"))
    ...
    def _save_state(self):
        self._save_job = None
        config.save_json(config.user_global_file("SKIPPER"),
